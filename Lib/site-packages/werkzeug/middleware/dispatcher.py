...
This example dispatches to an API controller under "/api", an admin controller
under "/admin", and an controller that serves frontend files for all other
requests::

    controller = DispatcherMiddleware(serve_frontend, {
        '/api': api_app,
        '/admin': admin_app,
    })
